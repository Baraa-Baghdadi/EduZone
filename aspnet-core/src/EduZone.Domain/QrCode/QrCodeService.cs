...
namespace EduZone.QrCode
{
    public class QrCodeService : ITransientDependency
    {
        ...
        public async Task<string> GenerateQrCode(Guid instructorId)
        {
            string Dawaa24LogoPath = Path.Combine(_Environment.WebRootPath, EduZoneConsts.EmailingFolderName, _configuration.Value["EmailTemplateSetting:Logo"]);
            var path = Path.Combine(_Environment.WebRootPath, EduZoneConsts.FilesFolderName);
            // this image we need to add it in middle barcode
            byte[] image = System.IO.File.ReadAllBytes(Dawaa24LogoPath);
            QRCodeGenerator qRGenerator = new QRCodeGenerator();
            QRCodeData qRCodeData = qRGenerator.CreateQrCode(instructorId + "", QRCodeGenerator.ECCLevel.Q);
            ...
            using (var ms2 = new MemoryStream(byteImage))
            {
                ...
                image1.Save(Path.Combine(path, "qr_code" + instructorId + ".png"));
            }
            return Path.Combine("qr_code" + instructorId + ".png");
        }
    }
}
